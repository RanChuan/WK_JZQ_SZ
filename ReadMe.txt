	...
	本程序适用的电路板版本：WK_JZQ 石柱 2018.12.15
...
2018.12.11
	石柱项目定制程序，上位机通信测试通过
	在通过配置软件配置的IP地址可以使用的情况下会使用用户配置的IP地址联网
	在通过配置软件配置的IP地址不能使用时会自动获取可用的IP地址
	使用原版的上位机通信协议，但是在配置软件给定的服务器IP地址不可用时会自动寻找服务器地址，原版的上位机软件是不支持的
	使用15寸屏，原版串口屏界面，在节能时间段不能自动息屏亮屏，后来不知为什么又可以了，一脸懵逼
2019.1.2
	无线信道可以通过网口配置，网口的系统信息加入无线信道显示
	消息队列满之后自动清除消息队列
